...
class ScriptedProviderTransport:
        ...
        return _completion(
            {
                ...
                "decision_reason": "Need to check both inventory data and policy.",
    ...
    def chat_post(
        ...
        if "risk_summary" in system:
            ...
            return _completion(
                {
                    "risk_summary": "Current inventory is below safety stock.[D1]",
                    "policy_basis": "Inventory policy requires triggering replenishment evaluation.[E1]",
                    "recommended_actions": ["Execute replenishment evaluation."],
                    "citations": ["[D1]", "[E1]"],
                }
            )
...
async def build_offline_controlled_inventory_runtime(
    ...
    *,
    ...
    knowledge_reviewer._post = lambda query, context: _completion(  # type: ignore[method-assign]
        {
            ...
            "missing_information": None,
            "decision_reason": "Selected evidence directly explains inventory policy.",
        }
    )
    ...
    data_planner._post = lambda query, schema, definitions: _completion(  # type: ignore[method-assign]
        {
            "status": "ready",
            "intent": "Query inventory status",
            "sql": "SELECT product_id, on_hand_quantity FROM inventory_snapshots",
            "decision_reason": "Inventory fields are authorized.",
            "missing_information": None,
        }
    )
    ...
    data_answer._post = lambda query, evidence: _completion(  # type: ignore[method-assign]
        {"answer": "Current inventory is 8, safety stock is 20.[D1]", "citations": ["[D1]"]}
    )
    data_tool = DataAgentTool(
...
def _write_inventory_dataset(tmp_path: Path) -> Path:
    ...
    generated.mkdir()
    content = "Replenishment evaluation is required when inventory falls below safety stock, and replenishment is scheduled according to the inventory policy."
    common = {
        "schema_version": "1.0",
