...
class DeterministicSubSkill:
    ...
    async def execute(self, *, user_query: str, decision: RouterDecision) -> SkillResult:
        ...
        answer = (
            "Product A current inventory is below safety stock; attention to replenishment risk is required. [D1]"
            if decision.route is RequestRoute.DATA
            else "The company replenishment policy requires prompt reordering when inventory drops below safety stock. [E1]"
        )
        return SkillResult(
...
async def test_real_provider_synthesizes_fake_inventory_evidence_once() -> None:
    ...
    decision = RouterDecision(
        ...
        normalized_query="Product A inventory is low; provide recommendations based on the replenishment policy",
        decision_reason="Requires inventory data and replenishment policy evidence",
        data_subquery="Is Product A current inventory below safety stock?",
        knowledge_subquery="What are the company replenishment policy requirements when inventory is below safety stock?",
    ...
    assert result.answer is not None and "Risk Overview:" in result.answer
