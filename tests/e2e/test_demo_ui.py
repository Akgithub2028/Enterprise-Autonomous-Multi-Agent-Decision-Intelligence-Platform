"""Browser-facing HTTP acceptance for the Module 7 demonstration surface."""
...
class _DeterministicRouter:
    def __init__(self, query: str, *, route: RequestRoute = RequestRoute.KNOWLEDGE) -> None:
        self.query = query
        self.route = route
        self.calls = 0
    ...
    async def route_with_context(
        ...
        *,
        ...
        return RouterDecision(
            ...
            decision_reason="deterministic_module_7_e2e",
            knowledge_subquery=(self.query if self.route is RequestRoute.KNOWLEDGE else None),
            confidence=1,
        )
...
class _Reviewer:
    ...
    async def review(self, **_: object) -> AnswerabilityDecision:
        ...
        return AnswerabilityDecision(
            ...
            decision_reason="Selected evidence clearly supports the answer.",
        )
...
class _KnowledgeRuntime:
    ...
    async def initialize(self) -> None:
        ...
        if self.fail_initialize:
            raise RuntimeError(
                "PRIVATE_UI_STARTUP_MARKER https://private.invalid PRIVATE_DATASET_MARKER"
            )
...
class _ConfiguredBoundaries:
    def __init__(
        ...
        *,
        ...
        route: RequestRoute = RequestRoute.KNOWLEDGE,
    ) -> None:
        self.router = _DeterministicRouter(query, route=route)
    ...
    def redis(self, _: str, __: float) -> object:
        raise AssertionError("In-memory E2E must not create Redis")
...
def test_demo_ui_and_formal_runtime_share_one_successful_application() -> None:
    ...
    with TestClient(app) as client:
        ...
        response = client.post(
            ...
            json={
                "request_id": "module-7-success",
                "session_id": "module-7-session",
                "query": query,
            },
        )

        assert root.status_code == styles.status_code == script.status_code == 200
        assert "<title>Enterprise Decision Agent</title>" in root.text
        ...
        assert response_body == {
            ...
            "citations": ["[E1]"],
            "error_code": None,
            "memory_context_status": "empty",
            "memory_persistence_status": "persisted",
            "memory_summarization_status": "not_requested",
        }
        assert trace["final_status"] == "completed"
        assert trace["request_id"] == "module-7-success"
        assert all("query" not in str(stage) for stage in trace["stages"])
...
def test_demo_ui_survives_runtime_startup_failure_and_rollback() -> None:
    query = "What is the Product A battery warranty?"
    ...
    with TestClient(app) as client:
        ...
        response = client.post(
            "/api/v1/agent/execute",
            json={
                "request_id": "module-7-startup-failed",
                "session_id": "module-7-session",
                "query": query,
            },
        )
    ...
    public_text = f"{root.text} {health.text} {ready.text} {response.text}".lower()
    assert all(
        marker not in public_text
        for marker in (
            "private_ui_startup_marker",
            "private.invalid",
...
def test_demo_ui_formal_request_preserves_unsupported_as_a_non_failure() -> None:
    query = "Tell me a joke"
    boundaries = _ConfiguredBoundaries(query, route=RequestRoute.UNSUPPORTED)
    settings = _settings()
    app = create_bootstrapped_app(
        settings,
        create_configured_runtime_builder(settings, boundaries.dependencies()),
    ...
    with TestClient(app) as client:
        response = client.post(
            "/api/v1/agent/execute",
            json={
                "request_id": "module-7-unsupported",
                "session_id": "module-7-session",
                "query": query,
            },
        )
    ...
    assert response_body == {
        ...
        "memory_summarization_status": "skipped",
    }
    assert trace["final_status"] == "unsupported"
    assert trace["request_id"] == "module-7-unsupported"
    assert boundaries.router.calls == 1
    assert boundaries.native.calls == boundaries.selector.calls == 0
    assert boundaries.reviewer.calls == boundaries.generator.calls == 0
    assert boundaries.knowledge.pipeline.queries == []
