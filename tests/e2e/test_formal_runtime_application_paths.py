    ...
    async def plan(
        ...
        *,
        ...
        return DataQueryPlan(
            ...
            intent="Query inventory risk",
            sql=self._sql,
            decision_reason="Read-only query conditions are complete.",
        )
...
class _RecordingSynthesizer:
    ...
    async def synthesize(
        ...
        return InventoryRiskSynthesisResult(
            ...
            recommended_actions=("Review according to policy and replenish inventory.",),
            citations=("[D1]", "[E1]"),
        )
...
def _decision(
    ...
    *,
    ...
    return RouterDecision(
        route=route,
        normalized_query=knowledge_subquery or data_subquery or "Inventory risk diagnosis",
...
async def test_formal_runtime_executes_data_application_path() -> None:
    query = "Query current inventory risk products"
...
async def test_formal_runtime_executes_mixed_application_path() -> None:
    data_query = "Query current inventory risk products"
    knowledge_query = "Explain inventory replenishment policy"
    sql = "SELECT product_id, on_hand_quantity FROM inventory_snapshots LIMIT 1"
    client = _DeterministicEnterpriseDataClient(result=_successful_data_result(sql))
    data_tool, planner, generator, factory = _data_tool(
        sql=sql,
...
async def test_formal_runtime_projects_safe_query_rejection() -> None:
    query = "Verify dangerous inventory write operation is rejected"
