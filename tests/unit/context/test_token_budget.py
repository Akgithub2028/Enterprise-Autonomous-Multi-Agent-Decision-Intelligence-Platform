...
def test_conservative_estimator_is_deterministic_for_empty_english_unicode_and_mixed_text() -> None:
    estimator = ConservativeCharacterTokenEstimator()

    assert estimator.estimate("") == 0
    assert estimator.estimate("abcd") == 1
    assert estimator.estimate("éñ") == 2
    assert estimator.estimate("Aéñ") == 3
    assert estimator.estimate("Aéñ") == estimator.estimate("Aéñ")
