...
def _evidence_context() -> EvidenceContext:
    child = MatchedChild(
        ...
        parent_id="PARENT-1",
        document_id="DOC-1",
        content="Product A original battery warranty period is 12 months.",
        upstream_rank=1,
    )
    item = EvidenceItem(
        evidence_id="E1",
    ...
    return EvidenceContext(
        rendered_context="[E1]\nProduct A original battery warranty period is 12 months.",
        evidence_items=(item,),
        references=(reference,),
        included_evidence_count=1,
        omitted_evidence_count=0,
        total_original_chars=56,
        total_included_chars=56,
        truncated=False,
    )
...
def _answerable_decision() -> AnswerabilityDecision:
    return AnswerabilityDecision(
        ...
        decision_reason="The selected evidence explicitly specifies the original battery warranty period.",
    )
...
def _unanswerable_decision() -> AnswerabilityDecision:
    return AnswerabilityDecision(
        ...
        missing_information="Additional free warranty period after repair completion",
        decision_reason="The selected evidence does not specify the additional free warranty period after repair completion.",
    )


def _answer_draft() -> AnswerDraft:
    return AnswerDraft(
        answer="Product A original battery warranty period is 12 months.[E1]",
        citations=["[E1]"],
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (_answer_draft().model_copy(update={"answer": "No inline citations."}), False, ["[E1]"]),
        (
            _answer_draft().model_copy(update={"answer": "Answer uses undeclared citation.[E2]"}),
            False,
            ["[E1]"],
        ),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"answerability": Answerability.UNANSWERABLE, "decision_reason": "reason"},
...
def test_state_rejects_invalid_running_reviewed_and_failed_states(
    ...
    with pytest.raises(ValidationError, match=error_match):
        KnowledgeQAState(user_query="test query", **state_update)


def test_state_accepts_selected_reviewed_and_final_contracts() -> None:
    selected = KnowledgeQAState(
        user_query="test query", selected_evidence=_evidence_context().evidence_items
    )
    ...
    final_unanswerable = reviewed_unanswerable.model_copy(
        update={"answer": "Existing evidence is insufficient to determine."}
    )
...
async def test_answerable_path_uses_only_selected_evidence_and_reviewer_decision() -> None:
    ...
    result = await run_knowledge_qa(
        ...
        user_query="How long is the original battery warranty for Product A?",
    )
...
async def test_reviewer_unanswerable_skips_generator_and_does_not_expose_missing_number() -> None:
    ...
    generator = RecordingGenerator()
    result = await run_knowledge_qa(
        _graph(reviewer=reviewer, generator=generator), user_query="Repair question?"
    )

    assert result.answerability is Answerability.UNANSWERABLE
    assert result.citations == []
    assert result.missing_information == "Additional free warranty period after repair completion"
...
async def test_unanswerable_answer_does_not_echo_a_reviewer_missing_number() -> None:
    reviewer = RecordingReviewer(
        AnswerabilityDecision(
            answerability="unanswerable",
            missing_information="30-day period after repair completion",
            decision_reason="The selected evidence does not specify the new period after repair completion.",
        )
    )
    result = await run_knowledge_qa(
...
async def test_reviewer_failure_retains_verified_selected_evidence_for_audit() -> None:
    result = await run_knowledge_qa(
        _graph(reviewer=RecordingReviewer(fails=True)), user_query="test query"
    )
...
async def test_selector_provider_subcode_is_safe_and_clears_unverified_selection() -> None:
    ...
    result = await run_knowledge_qa(
        _graph(selector=SelectorWithProviderFailure()), user_query="test query"
    )
...
async def test_invalid_generator_output_enters_safe_failed_state(
    ...
    result = await run_knowledge_qa(_graph(generator=generator), user_query="test query")
