...
async def test_reader_initialization_does_not_embed_or_upsert_the_formal_corpus() -> None:
    ...
    first = await reader.retrieve("Product A original battery warranty period")
    second = await reader.retrieve("Procurement contract approval requirements")
...
async def test_retrieve_requires_initialize() -> None:
    ...
    with pytest.raises(RetrievalValidationError, match="must be initialized"):
        await pipeline.retrieve("Product A battery warranty")
...
async def test_multiple_queries_reuse_indexes_and_models(initialized_pipeline) -> None:
    ...
    await pipeline.retrieve("Product A original battery warranty period")
    await pipeline.retrieve("Procurement contract approval requirements")
...
async def test_all_formal_children_survive_child_level_rrf_and_parent_aggregation() -> None:
    ...
    result = await pipeline.retrieve("enterprise")
...
async def test_result_mutation_does_not_change_pipeline_snapshot(initialized_pipeline) -> None:
    pipeline, _, _, _ = initialized_pipeline
    first = await pipeline.retrieve("Product A original battery warranty period")
    ...
    second = await pipeline.retrieve("Product A original battery warranty period")
...
async def test_runtime_ignores_missing_or_replaced_ground_truth(tmp_path: Path) -> None:
    ...
    await pipeline.initialize()
    without_ground_truth = await pipeline.retrieve("Product A original battery warranty period")
