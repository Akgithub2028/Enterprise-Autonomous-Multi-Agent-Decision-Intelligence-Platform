...
async def test_document_batch_is_passed_to_encode_without_instruction() -> None:
    ...
    await make_provider(model).embed_documents(["first document", "second document"])

    assert model.encode_calls[0][0] == ["first document", "second document"]
...
async def test_query_instruction_is_concatenated_exactly_once() -> None:
    ...
    original_query = "how long is the product battery warranty"

    await provider.embed_query(original_query)
    await provider.embed_query(f"{instruction}already prefixed")

    assert model.encode_calls[0][0] == [f"{instruction}{original_query}"]
    assert model.encode_calls[1][0] == [f"{instruction}already prefixed"]
    assert original_query == "how long is the product battery warranty"
