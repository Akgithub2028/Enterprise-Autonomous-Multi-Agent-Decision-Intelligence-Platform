...
def builder(
    total: int = 200, item: int = 100, count: int = 5, marker: str = "…[truncated]"
...
def test_character_budget_counts_python_characters() -> None:
    result = builder(item=4, marker="…").build([parent("p1", 1, "abcdef")])
    assert result.evidence_items[0].content == "abc…" and result.total_included_chars == 4
