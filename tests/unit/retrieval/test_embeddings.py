...
@pytest.mark.asyncio
async def test_related_domain_text_has_higher_similarity_than_unrelated_text() -> None:
    provider = DeterministicHashEmbeddingProvider(dimension=256)
    query = await provider.embed_query("inventory safety stock replenishment")
    related = await provider.embed_query("reorder trigger and safety stock policy")
    unrelated = await provider.embed_query("employee holiday travel reimbursement")
    assert cosine(query, related) > cosine(query, unrelated)
