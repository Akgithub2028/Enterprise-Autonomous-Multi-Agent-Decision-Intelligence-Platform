...
def corpus_row(document_id: str = "doc-1", **overrides: object) -> dict[str, object]:
    row: dict[str, object] = {
        ...
        "text": "Product battery warranty period is one year.",
        "category": "products_after_sales",
...
def query_row(query_id: str = "query-1", **overrides: object) -> dict[str, object]:
    row: dict[str, object] = {
        "query_id": query_id,
        "query": "How long is the product battery warranty?",
...
def test_load_corpus_returns_typed_ordered_documents(tmp_path: Path) -> None:
    ...
    write_jsonl(path, [corpus_row("doc-1"), corpus_row("doc-2", text="Second document")])

    documents = load_corpus(path)

    assert [document.document_id for document in documents] == ["doc-1", "doc-2"]
    assert documents[0].text == "Product battery warranty period is one year."


def test_load_queries_returns_typed_ordered_queries(tmp_path: Path) -> None:
    path = tmp_path / "queries.jsonl"
    write_jsonl(path, [query_row("query-1"), query_row("query-2", query="Second question")])
