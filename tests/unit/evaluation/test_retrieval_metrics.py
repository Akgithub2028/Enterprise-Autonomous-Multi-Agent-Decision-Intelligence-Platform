...
def test_macro_average_is_arithmetic_mean() -> None:
    overall, _ = aggregate_metrics(
        [("sales", {"hit_rate_at_1": 1.0}), ("sales", {"hit_rate_at_1": 0.0})]
    )
...
def test_category_metrics_are_aggregated_independently() -> None:
    _, categories = aggregate_metrics(
        [
            ("sales", {"hit_rate_at_1": 1.0}),
            ("sales", {"hit_rate_at_1": 0.0}),
            ("human_resources", {"hit_rate_at_1": 1.0}),
        ]
    )
    assert categories["sales"]["hit_rate_at_1"] == 0.5
    assert categories["human_resources"]["hit_rate_at_1"] == 1.0
