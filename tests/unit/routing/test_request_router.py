...
def test_router_prompt_includes_enterprise_profile_and_agent_boundary_classification() -> None:
    ...
    assert "Huaheng" not in ROUTER_SYSTEM_PROMPT
...
@pytest.mark.parametrize(
    ...
    [
        ("What is this enterprise?", RequestRoute.KNOWLEDGE),
        ("What does this company primarily do?", RequestRoute.KNOWLEDGE),
        ("Is this a real enterprise?", RequestRoute.KNOWLEDGE),
        ("What can you do?", RequestRoute.KNOWLEDGE),
        ("What enterprise knowledge do you possess?", RequestRoute.KNOWLEDGE),
        ("Can you search the web?", RequestRoute.KNOWLEDGE),
...
async def test_router_parses_capability_boundary_decisions_without_executing_capabilities(
    ...
    assert "Huaheng" not in decision.model_dump_json()
...
async def test_router_request_uses_strict_json_and_limited_system_context(
            ...
            content = json.dumps(
                {
                    ...
                    "decision_reason": "Question asks about enterprise policy.",
    ...
    decision = await _router(base_url).route(user_query="What is the procurement approval process?")
        ...
        (
            _payload(
                content=json.dumps(
                    {
                        "route": "unsupported",
                        "normalized_query": "Delete all products",
                        "decision_reason": "Write actions are not supported.",
