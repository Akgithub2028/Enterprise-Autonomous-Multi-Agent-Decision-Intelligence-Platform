...
def input_data() -> InventoryRiskSynthesisInput:
    return InventoryRiskSynthesisInput(
        original_request="Inventory risk and replenishment policy",
        data_subquery="Which products have insufficient inventory",
        data_answer="Product A inventory is below safety stock.[D1]",
        data_citations=("[D1]",),
        knowledge_subquery="What is the company replenishment policy",
        knowledge_answer="Policy requires timely replenishment.[E1]",
...
def valid_content() -> str:
    return json.dumps(
        {
            "risk_summary": "Product A has inventory risk.",
...
async def test_synthesizer_uses_one_plain_json_request_with_minimal_prompt() -> None:
    ...
    assert "Product A inventory is below safety stock.[D1]" in prompt
    assert "Policy requires timely replenishment.[E1]" in prompt
...
async def test_standard_json_arrays_and_different_field_order_are_accepted() -> None:
    content = json.dumps(
        {
            "citations": ["[D1]", "[E1]"],
            "recommended_actions": ["Take replenishment action"],
            "policy_basis": "Policy requires timely replenishment.",
            "risk_summary": "Inventory risk exists.",
        },
        ensure_ascii=False,
    )
    ...
    assert result.recommended_actions == ("Take replenishment action",)
    assert result.citations == ("[D1]", "[E1]")
    assert len(client.calls) == 1
