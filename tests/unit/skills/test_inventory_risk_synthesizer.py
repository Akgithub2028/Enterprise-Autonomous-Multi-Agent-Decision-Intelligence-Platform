...
def mixed() -> RouterDecision:
    return RouterDecision(
        ...
        normalized_query="Inventory risk and replenishment policy",
        decision_reason="Requires data and policy evidence",
        data_subquery="Which products have insufficient inventory",
        knowledge_subquery="What is the company replenishment policy",
...
def synthesis(citations: tuple[str, ...] = ("[D1]", "[E1]")) -> InventoryRiskSynthesisResult:
    return InventoryRiskSynthesisResult(
        risk_summary="Inventory is below safety stock.",
        policy_basis="Policy requires timely replenishment.",
        recommended_actions=("Schedule replenishment.", "Track inventory."),
        citations=citations,
    )
...
def test_synthesis_models_are_strict_immutable_and_evidence_bounded() -> None:
    input_data = InventoryRiskSynthesisInput(
        original_request="Inventory risk",
        data_subquery="Inventory query",
        data_answer="Data conclusion",
        data_citations=("[D1]",),
        knowledge_subquery="Replenishment policy",
        knowledge_answer="Policy conclusion",
...
async def test_synthesizer_receives_only_trusted_public_input_once() -> None:
    ...
    assert input_data.original_request == "Inventory risk and replenishment policy"
...
async def test_valid_synthesis_is_rendered_with_stable_deduplicated_citations() -> None:
    ...
    assert result.answer == (
        ...
        "Policy Basis:\nPolicy requires timely replenishment.\n\n"
        "Recommended Actions:\n1. Schedule replenishment.\n2. Track inventory.\n\n"
        "Citations:\n[D1] [E1]"
    )
