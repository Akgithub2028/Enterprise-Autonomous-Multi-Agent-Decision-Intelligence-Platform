...
def mixed(
    ...
    return RouterDecision(
        ...
        normalized_query="Inventory risk diagnosis",
        decision_reason="Requires data and policy evidence",
        data_subquery=data,
...
class FakeSynthesizer:
    async def synthesize(
        ...
        return InventoryRiskSynthesisResult(
            risk_summary="Inventory risk exists.[D1]",
            policy_basis="Policy requires timely replenishment.[E1]",
            recommended_actions=("Schedule replenishment.",),
            citations=("[D1]", "[E1]"),
        )
...
def completed(route: RequestRoute) -> SkillResult:
    ...
    return SkillResult(
        ...
        skill_version="1.0.0",
        route=route,
        answer="Trusted answer" + citation,
        citations=[citation],
        executed_steps=("fake",),
        selected_tool=tool,
    )
...
@pytest.mark.parametrize("query", ["Supplier performance and procurement policy", "hello"])
def test_irrelevant_mixed_does_not_apply(query: str) -> None:
    skill, _, _, _ = make_skill()
    assert skill.is_applicable(query, mixed("sales amount", "procurement policy")) is False


@pytest.mark.parametrize(
    ("data", "knowledge"),
    [(None, "replenish"), ("inventory", None), ("   ", "replenish"), ("inventory", "   ")],
)
def test_malformed_mixed_subqueries_do_not_apply(data: str | None, knowledge: str | None) -> None:
    ...
    skill, _, _, _ = make_skill()
    assert skill.is_applicable("inventory", malformed) is False
...
def test_non_mixed_decision_does_not_apply(route: RequestRoute) -> None:
    decision = RouterDecision(
        ...
        data_subquery="Inventory data" if route is RequestRoute.DATA else None,
        missing_information="Unsupported" if route is RequestRoute.UNSUPPORTED else None,
        confidence=0.9,
    )
    skill, _, _, _ = make_skill()
    assert skill.is_applicable("Inventory risk", decision) is False
...
async def test_data_then_knowledge_subdecisions_are_isolated_and_ordered() -> None:
    ...
    assert "Risk Overview:" in result.answer
...
async def test_invalid_data_result_short_circuits_knowledge(data_result: SkillResult) -> None:
    ...
    result = await skill.execute(user_query="Inventory risk", decision=mixed())
...
async def test_control_flow_exceptions_are_not_swallowed(error: BaseException) -> None:
    ...
    with pytest.raises(type(error)):
        await skill.execute(user_query="Inventory risk", decision=mixed())
