...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--query", required=True, type=_nonblank_query, help="Enterprise knowledge query to retrieve"
    )
    parser.add_argument(
        "--dataset-root",
        type=Path,
        default=ROOT / "datasets/enterprise_kb/m2c1",
        help="Enterprise KB dataset directory containing generated/",
    )
    return parser
...
async def _run(args: argparse.Namespace) -> int:
    ...
    print("=== Initialization Latency (seconds) ===")
    ...
    print("=== Query Stage Latency (seconds) ===")
    for name, value in result.stage_timings.model_dump().items():
        print(f"{name}={value:.6f}")
    print("Retrieval evidence only, no LLM answer generated")
    return 0
