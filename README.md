...
flowchart TD
    ...
    subgraph SecurityContextLayer["Context, Memory & Security Boundary"]
        API --> EX["Request Executor"]
        EX --> CM["Context & Token Budget Manager"]
        CM <--> MEM["Redis 7+ / In-Memory Session Store"]
        CM --> SEC["Security & Scope Principal (DataScope / KnowledgeScope)"]
    end

    subgraph RoutingLayer["Multi-Agent Supervisor"]
        SEC --> R["Dynamic Request Router (LangGraph)"]
        R --> C["Coordinator & Skill Dispatcher"]
    end

    subgraph ExecutionLayer["Specialized Agent Workflows"]
        C -->|Route: Knowledge| K["Knowledge QA Agent"]
        C -->|Route: Data| D["Data Analytics Agent"]
        C -->|Route: Mixed| M["Strategic Decision Agent"]

        subgraph HybridRAGPipeline["Clause-Aware Hybrid RAG"]
            K --> RAG["Dense (Milvus HNSW) + Sparse (BM25)"]
            RAG --> RRF["Reciprocal Rank Fusion (RRF)"]
            RRF --> RERANK["BGE Cross-Encoder Reranker"]
            RERANK --> PARENT["Parent-Child Context Expansion"]
        end

        subgraph MCPDataPipeline["Guarded MCP NL2SQL Engine"]
            D --> DPLAN["Autonomous Data Query Planner"]
            DPLAN --> MCP["Model Context Protocol (MCP) Server"]
            MCP --> SQLG["SQLGlot AST Validator & Read-Only Guard"]
            SQLG --> MYSQL[("Enterprise MySQL (Replica)")]
        end

        subgraph JointSynthesis["Decision Synthesis & Diagnosis"]
            M --> KT["Knowledge Evidence Subtask"]
            M --> DT["Operational Data Subtask"]
            KT --> SYN["Joint Risk & Policy Synthesizer"]
            DT --> SYN
        end
    end

    subgraph AuditReviewLayer["Verification & Output Release"]
        PARENT --> REV["Evidence Answerability Reviewer"]
        MYSQL --> REV
        SYN --> REV
        REV -->|Pass: Grounded & Sufficient| OUT["Verified Decision Release + Exact Citations ([E1], [D1])"]
        REV -->|Fail: Insufficient Evidence| CLAR["Structured Fallback / Refusal Plan"]
        EX -.-> TRACE["Structured Traces & Latency Metrics"]
    end
```
